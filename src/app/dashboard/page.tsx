...
import { Layout, Pencil, Plus, Trash } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const uniqueBoards = boards
    ? Array.from(new Map(boards.map((board) => [board._id, board])).values())
    : [];
  ...
  const handleJoinBoard = async () => {
    const id = prompt("Masukkan ID Board temen kamu:");
    if (!id) return;
        ...
        {boards === undefined ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {uniqueBoards.map((board) => (
              <div
                key={board._id}
                className="group relative bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-indigo-300 transition-all h-48 flex flex-col justify-between"
              >
                ...
                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={(e) => {
                      ...
                      if (newTitle)
                        updateBoard({ id: board._id, title: newTitle });
                    }}
                  ...
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      if (confirm("Yakin hapus board ini?"))
                        removeBoard({ id: board._id });
                    }}
